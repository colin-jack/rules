...
var throwIfKeyOrValueInappropriate = function(key, value) {
    if (typeof value !== 'number') {
        throw new RulesError("The value passed as the second argument to now must be a moment/date/date string.")
    }

    if (_u.contains(SupportedKeys, key) === false) {
        throw new RulesError("The key is not supported. Only " + SupportedKeys.join('/') + " supported.");
    }
};
