...
var validateConfig = function(config) {
    if (unsuitableConstraint(config.minLength)) {
       throw new RulesError("The minimum length (minLength) must be a number.");
    }

    if (unsuitableConstraint(config.maxLength)) {
       throw new RulesError("The maximum length (maxLength) must be a number.");
    }

    if ((config.minLength !== undefined && config.maxLength !== undefined) && config.minLength >= config.maxLength) {
        throw new RulesError("The minimum length (minLength) must be less than maximum (maxLength).");   
    }
}
